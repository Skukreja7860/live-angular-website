...
@Component({
  selector: 'app-learnerupdate',
  templateUrl: './learnerupdate.component.html',
  styleUrls: ['./learnerupdate.component.css']
})
export class LearnerupdateComponent implements OnInit {
  ...
  onupdate() {
    ...
    this.myhttp.put("http://localhost:3000/api/learnerupdate", data, { responseType: "json" }).subscribe(
      {
        next: (resp) => {
          if (resp["nModified"] == 1) {
