...
@Component({
  selector: 'app-learneredit',
  templateUrl: './learneredit.component.html',
  styleUrls: ['./learneredit.component.css']
})
export class LearnereditComponent implements OnInit {
  name: string;
  gender: string;
  address: string;
  ...
  onsave() {
    ...
    this.myhttp.post("http://localhost:3000/api/learneredit", data, { responseType: "text" }).subscribe(
      {
        next: (resp) => {
          this.msg = resp;
          alert(this.msg);
          this.myrouter.navigateByUrl("/learnerdata");
        },
        error: (err) => {
          this.msg = err;
        }
